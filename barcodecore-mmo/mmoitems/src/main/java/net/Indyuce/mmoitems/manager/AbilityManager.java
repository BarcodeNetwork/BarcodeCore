...
public class AbilityManager implements Reloadable {
	...
	public void registerAbilityByExternal(Ability ability) {
		Validate.isTrue(!hasAbility(ability.getID()), "An ability is already registered with the same ID");
	...
	public void reload() {
		...
		if (Bukkit.getPluginManager().getPlugin("MythicMobs") != null) {
			...
			for (File file : mythicMobs.listFiles())
				try {
					Ability ability = new MythicMobsAbility(file.getName().substring(0, file.getName().length() - 4), YamlConfiguration.loadConfiguration(file));
					abilities.put(ability.getID(), ability);
					count++;
